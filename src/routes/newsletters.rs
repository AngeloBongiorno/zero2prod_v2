use actix_web::http::{StatusCode, header::{HeaderMap, HeaderValue}};
...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    ...
    let username = credentials
        .next()
        .ok_or_else(|| {
            anyhow::anyhow!("A username needs to be provided in 'Basic' auth.")
        })?
        .to_string();
    ...
    let password = credentials
        .next()
        .ok_or_else(|| {
            anyhow::anyhow!("A password needs to be provided in 'Basic' auth.")
        })?
        .to_string();
